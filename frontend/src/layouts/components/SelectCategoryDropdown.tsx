    ...
    <div className={"w-full relative"}>
      ...
      <ul className={`absolute z-10 w-full mt-2 p-0 form-input overflow-y-auto ${open ? "max-h-60" : "hidden"}`}>
        ...
        {categoryNames?.map((category, index) => (
          <li
            key={index}
            className={`p-2 mx-4 mb-1 cursor-pointer ${category === selectedCategory && "bg-gray-200 dark:bg-gray-700"} hover:bg-gray-200 dark:hover:bg-gray-700 rounded ${category?.toLowerCase().startsWith(inputValue) ? "block" : "hidden"}`}
            onClick={() => {
